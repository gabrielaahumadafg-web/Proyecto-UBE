...
| GET | `/disponibilidad` | public | Available blocks for a service (`?id_servicio=`). Optional `?ventana_dias=N` caps the result to blocks within the next N days. **The student booking flow passes `ventana_dias=14`** (paradigm: a student can only see/book within the next 2 weeks — beyond that a slot is *only* reachable via the waitlist, even if the same day+hour is fully occupied now). Staff (admin/coordinador/profesional) **omit** the param to schedule without a cap; when omitted, the historical 14-day cap still applies to **cyclic** services only (whose weekly series would otherwise return thousands of rows). Callers passing `ventana_dias`: `AgendarHora.jsx` (student booking) and `Dashboard.jsx` (student waitlist editor). The student grid (`AgendarHora.jsx`) also disables week navigation outside the 2-week window. |
...
| GET | `/reportes/espera_comparativa` | prof_apoyo, coordinador, admin | **Average total wait per attended student**, split into two **additive** legs (`?fecha_inicio&fecha_fin&id_servicio`). One data point per `proceso` = its **earliest already-past, non-cancelled** cita (avoids inflating the average with later cyclic sessions). For that reserva: `t_reserva` = cita − `reserva.fecha_creacion`; `t_espera` = `reserva.fecha_creacion` − `reserva.fecha_ingreso_lista` (0 when null = direct booking). `t_total = t_reserva + t_espera`. Date range filters by cita (`bloque_horario.fecha_hora_inicio`). Returns `{cantidad, promedio_total, reserva:{promedio,porcentaje}, lista_espera:{promedio,porcentaje}, datos_lista_espera_disponibles}` (so `reserva.promedio + lista_espera.promedio ≈ promedio_total`). Powers the "Espera Promedio por Estudiante" stacked bar. **Waitlist leg depends on `reserva.fecha_ingreso_lista`** (nullable `timestamptz`, added by `Backend_UBE/migrations/add_fecha_ingreso_lista_reserva.sql`): the waitlist-join date is copied onto the reserva at assignment time (before the `lista_espera` row is deleted) in `_procesar_reserva_bloques(..., fecha_ingreso_lista)`. **Forward-only**: pre-migration attentions have no trace, so their waitlist leg = 0; if the column doesn't exist yet the endpoint falls back (all direct) and returns `datos_lista_espera_disponibles=false`. |
...
> The Profesional de Apoyo dashboard has an **"Ocupación / Espera"** tab (`pestañaActiva === 'ocupacion_espera'`, date-range picker defaulting to *year-start → today*) with two indicators: (1) **% de Ocupación por Especialidad** — a horizontal bar chart of occupancy % per service plus a general % badge, built by aggregating `/reportes/ocupacion` (per prof+service) by `id_servicio` **in the frontend** (no backend change); (2) **Espera Promedio por Estudiante (hasta ser atendido)** — a single horizontal **stacked** bar from `/reportes/espera_comparativa` splitting the average total wait into its lista-de-espera + reserva legs (they sum to the total), with side cards showing the total and each leg's average days + % of total. Only counts attentions already received; a footnote flags that the waitlist leg is forward-only (see the endpoint's `fecha_ingreso_lista` note).
...
- **`_procesar_reserva_bloques(id_proceso, id_bloque_final, fecha_ingreso_lista=None)`** — creates `reserva` rows. For cyclic services, reserves the full weekly series up to `tope_sesiones`. First block → `confirmado`; rest → `reservado`. All reservas start as `pendiente`. The optional `fecha_ingreso_lista` (the `lista_espera.fecha_ingreso` of a student being assigned **from the waitlist**) is best-effort stamped onto the created reserva(s) via `_guardar_fecha_ingreso_lista` (try/except, tolerates the column not existing) so `/reportes/espera_comparativa` can measure the waitlist leg later. Passed by the waitlist-assignment callers (`_attempt_automatic_assignment`, `_attempt_automatic_assignment_for_student`, `/asignar_hora_manual`, `/admin/agendar_hora` when the student was already waiting, `/responder_oferta`); left `None` for pure direct bookings (`/reservar`, professional derivations) whose wait starts at `fecha_creacion`.
...
- `getSubSlots(hora, duracionMin)` — returns the sub-slots `[{inicio, fin}]` for one hour-row of a grid, **chained continuously from 08:00** (the first hour of every service). Each slot lasts `duracionMin`; for durations that don't divide 60 (e.g. 45 min) the chain does **not** reset each hour — `08:00–08:45, 08:45–09:30, 09:30–10:15…` — and each slot is bucketed into the hour-row where it starts (a row may hold 0, 1 or 2 slots). Capped at 18:00 (no slot ends later). For durations that divide 60 (60/30/20/15) the output is identical to the old per-hour alignment, so those grids look unchanged. **All sub-slot grids call this** (booking, declared-availability, and the coordinator publish modal `getSubSlotsCrear`), so what the coordinator publishes lines up exactly with what students book, and block matching stays `substring(0,5) === inicio`.

### Block publishing (press-the-block model)
The coordinator publishes availability by **clicking a block (day + hour cell)** in the management grid. `DashboardCoordinador.jsx` opens the "Publicar Hora" modal whose `getSubSlotsCrear()` offers **fixed sub-slot buttons** (via `getSubSlots`, chained from 08:00) — the coordinator picks one, and `POST /bloques` generates the weekly series to Dec 31 (skipping holidays), enforcing the authoritative per-professional **overlap** check (`nb_ini < ex_fin and nb_fin > ex_ini`). Because everything is driven by the same `getSubSlots` chain, the times the coordinator publishes are exactly the times students can book.

> **History:** a "Google-Calendar" free-minute model (`<input type="time">` + `mergeSlotsConBloques`, commit `1f8935c`) was tried on 2026-07-05 and **reverted** the same day. Two professionals of the same service attending a few minutes apart are **not** "the same block", and the free-minute grid had visualization problems. The press-the-block model with continuous 08:00-chained sub-slots replaced it.

### Sub-hourly service grids
Booking/availability grids use 10 hourly rows (08:00–17:00). Each cell's sub-slots come from `getSubSlots(hora, duracionMin)` (see calendar utilities) — **chained continuously from 08:00**, so a 45-min service is selectable one after another (`08:00–08:45, 08:45–09:30, 09:30–10:15…`) instead of wasting the sub-hour remainder. 60/30/20/15-min services render exactly as before.

**Real-block booking grids** (those that display/reserve actual `bloque_horario`) match a cell's blocks to a sub-slot with `b.fecha_hora_inicio…substring(0,5) === inicio`. Because coordinators publish and students book off the **same** chain, that exact match is sufficient. These grids are:
...
**Waitlist / priority declared-availability grids** (student `AgendarHora.jsx` paso 3, admin priority grid, coordinator priority grid) render the same `getSubSlots` chain — they are for the student to **declare** availability, not to book a specific block. Backend waitlist matching is exact `strftime("%H:%M")`; since declared times and published blocks both follow the 08:00 chain, they line up.

The available `duracion_minutos` is read per-service (`servicio.duracion_minutos`, `servicios.find(...)?.duracion_minutos`, etc.), falling back to `60`. **Critical:** declared-availability grids must store the exact sub-slot start time (e.g. `"08:45"`) in `disponibilidad_indicada`, never the hour-level `"08:00"` — the backend matches by exact `strftime("%H:%M")`.
...
servicio (id_servicio, nombre, es_ciclico, tope_sesiones, duracion_minutos)
  └─ bloque_horario (id_bloque, id_profesional, id_servicio, fecha_hora_inicio, estado, id_ubicacion nullable → ubicacion)
       └─ reserva (id_reserva, id_proceso, id_bloque, estado, fecha_creacion, fecha_ingreso_lista nullable ← lista_espera.fecha_ingreso preservada al asignar desde la lista)
